...
let currentLogger: { client: PluginInput["client"]; directory: string } | undefined;
let pendingLogWrites: Promise<void> = Promise.resolve();
let debugEnabled = process.env.CURSOR_PROXY_DEBUG === "1";

export function configureLogger(input: PluginInput): void {
  currentLogger = { client: input.client, directory: input.directory };
}
...
export function errorDetails(error: unknown): Record<string, unknown> {
  ...
  return { errorType: typeof error, errorValue: serializeValue(error, 1) };
}

export function logDebug(message: string, extra: Record<string, unknown> = {}): void {
  if (debugEnabled) logPlugin("debug", message, extra);
}
...
function logPlugin(level: LogLevel, message: string, extra: Record<string, unknown>): void {
  const serializedExtra = serializeValue(extra, 0) as Record<string, unknown>;

  if (debugEnabled) writeConsoleLog(level, message, serializedExtra);

  if (level === "debug") return;
  if (!currentLogger?.client?.app?.log) return;
    ...
    .then(async () => {
      try {
        await currentLogger?.client.app.log({
          ...
          body: { service: PLUGIN_LOG_SERVICE, level, message, extra: serializedExtra },
        });
      } catch (logError) {
...
function writeConsoleLog(level: LogLevel, message: string, extra: Record<string, unknown>): void {
  const ts = new Date().toISOString().slice(11, 23);
  const prefix = `[proxy ${ts}] ${message}`;
